package com.learning.stream.portfolio;
...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        int amountOfDays = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
        ...
        double sizeOfList = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .mapToInt(tl -> tl.getTasks().size())
                .sum();

        // Then
        assertEquals(10, amountOfDays / sizeOfList);
        assertEquals(3, sizeOfList);
    }

    @Test
    void testAddTaskListAverageWorkingOnTaskSecondSolution() {
        // Given
        Board project = prepareTestData();
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
    ...
    private Board prepareTestData() {
        ...
        Task task6 = new Task("Use Stream",
